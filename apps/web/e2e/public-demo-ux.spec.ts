...
describeProductionUx("public demo UX", () => {
  test("demo shell uses product sidebar and single navigation", async ({ page }) => {
  ...
  test("technical details disclosure works", async ({ page }) => {
    ...
    await expect(page.getByTestId("technical-details").getByText(/google|gemini|openai/i).first()).toBeVisible();
  });
  ...
  test("locale switch reloads localized presentation", async ({ page, context }) => {
    ...
    await expect(page.getByTestId("demo-shell")).toBeVisible({ timeout: 20_000 });
    await expect(page.getByTestId("demo-notice")).toContainText(/ricerca dimostrativa|consultare liberamente/i);
  });
});
