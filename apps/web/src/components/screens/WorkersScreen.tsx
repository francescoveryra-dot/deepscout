...
import {
  presentArchitectureLabel,
  presentParentLabel,
} from "@/presentation/architecture";
import {
...
function kindLabel(kind: string, t: (key: string) => string): string {
  if (
    kind.includes("llm") ||
    kind.includes("agent") ||
    kind.includes("langgraph")
  )
    return t("workers.agentic");
...
function WorkerProgress({
  ...
  if (state === "completed") {
    return (
      <p className="worker-state-note completed">{t("phase.completed")}</p>
    );
  }
  if (state === "failed") {
  ...
  if (state === "blocked") {
    return (
      <p className="worker-state-note failed">
        {t(
          outcomeReason === "source_portfolio_inadequate"
            ? "workers.blockedPortfolio"
            : "workers.blockedNoEvidence",
        )}
      </p>
    );
  }
...
function WorkerCard({
  ...
  const title = presentWorkerCardTitle(
    workspace,
    item.worker_id,
    locale as "en" | "it",
  );
    ...
    >
      <div className="worker-card-header">
        <span className={`worker-badge ${tone}`}>
          {presentWorkerIndex(workspace, item.index, locale as "en" | "it")}
        </span>
      ...
      <WorkerProgress
        state={item.state}
        pct={pct}
        outcomeReason={item.outcome_reason}
        t={t}
      />
      <div className="worker-stats">
        <span>
          {item.agent_backed
            ? t("workers.agentic")
            : t("workers.deterministic")}
        </span>
        {item.retries > 0 ? (
          <span>
            {t("plan.retries")}: {item.retries}
          </span>
        ) : null}
...
export function WorkersScreen() {
  ...
  const worker =
    workspace.workers.find((item) => item.worker_id === selected) ??
    workspace.workers[0] ??
    null;
  const arch = workspace.architecture;
  const runningCount = workspace.workers.filter(
    (item) => item.state === "running",
  ).length;
  const completedCount = workspace.workers.filter(
            ...
            <div className="topo-stage-row">
              {arch.orchestrator ? (
                <div className="topo-node topo-node-stage completed">
                  <span className="topo-node-title">
                    {presentArchitectureLabel(
                      "orchestrator",
                      arch.orchestrator.label,
                      t,
                    )}
                  </span>
                  <span className="kind-tag deterministic">
                    {kindLabel(arch.orchestrator.kind, t)}
                  </span>
                </div>
              ) : null}
              ...
              {arch.planner ? (
                <div className="topo-node topo-node-stage completed">
                  ...
                  <span className="kind-tag agentic">
                    {kindLabel(arch.planner.kind, t)}
                  </span>
                </div>
              ) : null}
            </div>
            ...
            <div className="topo-workers-grid">
              {workspace.workers.map((item) => (
                <button
                  key={item.worker_id}
                  type="button"
                  className={`topo-node topo-node-worker ${item.state === "completed" ? "completed" : item.state === "running" ? "running" : item.state === "blocked" ? "blocked" : ""} ${worker?.worker_id === item.worker_id ? "selected" : ""}`}
      ...
      <div className="workers-layout">
        <section className="card workers-list-card">
          <div className="card-head">
            ...
            <div className="row">
              <span className="chip">
                {t("workers.total", { count: workspace.workers.length })}
              </span>
              {runningCount > 0 ? (
                <span className="chip selected">
                  {t("workers.runningCount", { count: runningCount })}
                </span>
              ) : null}
              <span className="chip">
                {t("workers.completedCount", { count: completedCount })}
              </span>
            </div>
          </div>
          {workspace.workers.length === 0 ? (
            <p className="empty">{t("workers.empty")}</p>
          ) : null}
          ...
          {worker ? (
            <>
              <div className="worker-detail-header">
                <span
                  className={`worker-badge ${workerTone(worker.index - 1)}`}
                >
