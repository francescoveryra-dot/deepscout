...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_provider_credentials_principal_id", "provider_credentials", ["principal_id"]
    )
    op.create_table(
        "auth_events",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
    ...
    op.add_column(
        "research_runs",
        sa.Column("is_public_demo", sa.Boolean(), nullable=False, server_default=sa.false()),
    )
    ...
    op.execute(
        sa.text("UPDATE research_runs SET owner_principal_id = :id").bindparams(id=LOCAL_SYSTEM_ID)
    )
    ...
    op.execute(
        sa.text("UPDATE research_templates SET owner_principal_id = :id").bindparams(
            id=LOCAL_SYSTEM_ID
        )
    )
    ...
    op.execute(
        sa.text("UPDATE research_monitors SET owner_principal_id = :id").bindparams(
            id=LOCAL_SYSTEM_ID
        )
    )
