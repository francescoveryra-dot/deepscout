...
class LearningExperienceSampleRow(Base):
    __tablename__ = "learning_experience_samples"
    __table_args__ = (
        Index("ix_learning_experience_owner_strategy", "owner_principal_id", "strategy_key"),
    )
...
class LearningPolicyMonitoringRow(Base):
    __tablename__ = "learning_policy_monitoring"
    __table_args__ = (Index("ix_learning_policy_monitoring_key", "policy_key", "status"),)

    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    policy_version_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("learning_policy_versions.id", ondelete="CASCADE")
    )
    policy_key: Mapped[str] = mapped_column(String(128), nullable=False)
    ...
    observed_samples: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    status: Mapped[str] = mapped_column(String(32), nullable=False, default="active")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())


class LearningExperimentJobRow(Base):
    __tablename__ = "learning_experiment_jobs"
    __table_args__ = (Index("ix_learning_experiment_jobs_status", "status"),)

    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    candidate_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("improvement_candidates.id", ondelete="CASCADE")
    )
    owner_principal_id: Mapped[uuid.UUID | None] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("principals.id", ondelete="CASCADE")
    )
